...
# ============================
$kioskUser = "Kiosk"
$kioskPassword = ConvertTo-SecureString "kiosk" -AsPlainText -Force
$appPath = "C:\Program Files (x86)\aktivKONZEPTE\aktivSYSTEM\aktivSYSTEM-User\WinUser.exe"

# ============================
# 1. Kiosk-Benutzer anlegen
# ============================
#Write-Host "Benutzer '$kioskUser' wird erstellt..."
#if (-not (Get-LocalUser -Name $kioskUser -ErrorAction SilentlyContinue)) {
#    New-LocalUser -Name $kioskUser -Password $kioskPassword -FullName "Kiosk Benutzer" -Description "Kiosk-Modus Benutzer" -PasswordNeverExpires -UserMayNotChangePassword
#    Add-LocalGroupMember -Group "Users" -Member $kioskUser
#} else {
#    Write-Host "Benutzer '$kioskUser' existiert bereits."
#}
...
Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name "DefaultPassword" -Value "kiosk"
