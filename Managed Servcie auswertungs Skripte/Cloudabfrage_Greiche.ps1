...
# In der Schleifen werden auch die einzelnen Werte zusammgerechnet. 
...
foreach($device in $devices){
    if($device.dnsName -match "DC01.gip.local"){
        $output = $device.id
   }       
}
$output
...
$rdsuserGreiche = $custom_field.rdsuserGreiche
...
Write-Host "VM Name: $vmName, CPU: $cpuCount, RAM: $memoryGB GB, Storage: $storageGB GB", $rdsuserGreiche "RDS-User"
C:\ProgramData\NinjaRMMAgent\ninjarmm-cli.exe set greicheCloudUmgebung  $Cloudvms "VMs," $CloudCPU "CPUs," $CloudRam "GB RAM," $CloudStorage "StorageGB,", $rdsuserGreiche "RDS-User"
